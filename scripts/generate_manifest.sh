...
KEY_ID="${PORTSIDE_MANIFEST_SIGNING_KEY_ID:?Set the configured manifest signing key ID}"
...
INPUT="$INPUT" KEY_FILE="$KEY_FILE" OUTPUT="$OUTPUT" KEY_ID="$KEY_ID" node --input-type=module <<'NODE'
...
input.signatureKeyId = process.env.KEY_ID;
