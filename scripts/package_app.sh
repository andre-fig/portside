...
STEAM_HOST_APP="$APP_DIR/Contents/Helpers/Steam.app"

swift build -c release --arch arm64 --product Portside --package-path "$ROOT_DIR"
swift build -c release --arch arm64 --product SteamHost --package-path "$ROOT_DIR"
rm -rf "$APP_DIR"
rm -rf "$DSYM_DIR"
rm -rf "$ICONSET_DIR"
...
mkdir -p "$STEAM_HOST_APP/Contents/MacOS" "$STEAM_HOST_APP/Contents/Resources"
cp "$ROOT_DIR/.build/arm64-apple-macosx/release/SteamHost" "$STEAM_HOST_APP/Contents/MacOS/Steam"
cp "$ROOT_DIR/Resources/SteamHostInfo.plist" "$STEAM_HOST_APP/Contents/Info.plist"
cp "$ROOT_DIR/Resources/Steam.icns" "$STEAM_HOST_APP/Contents/Resources/Steam.icns"
mkdir -p "$ICONSET_DIR"
...
chmod +x "$STEAM_HOST_APP/Contents/MacOS/Steam"
dsymutil "$APP_DIR/Contents/MacOS/Portside" -o "$DSYM_DIR"

SIGNING_IDENTITY="${PORTSIDE_CODESIGN_IDENTITY:--}"
if [ "$SIGNING_IDENTITY" = "-" ]; then
    codesign --force --sign - "$STEAM_HOST_APP"
    codesign --force --deep --sign - "$APP_DIR"
else
    codesign --force --options runtime --timestamp --sign "$SIGNING_IDENTITY" "$STEAM_HOST_APP"
