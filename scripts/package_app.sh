...
swift build -c release --arch arm64 --product PortsideInstaller --package-path "$DESKTOP_DIR"
rm -rf "$APP_DIR"
rm -rf "$DSYM_DIR"
rm -rf "$ICONSET_DIR"
...
cp "$DESKTOP_DIR/.build/arm64-apple-macosx/release/PortsideInstaller" "$APP_DIR/Contents/MacOS/PortsideInstaller"
