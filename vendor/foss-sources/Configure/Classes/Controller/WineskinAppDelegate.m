...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
	...
	NSImage *theImage = [[NSImage alloc] initByReferencingFile:[NSString stringWithFormat:@"%@/Contents/Resources/Configure.icns",self.wrapperPath]];
...
- (void)setButtonsState:(BOOL)state
{
    [NSThread dispatchBlockInMainQueue:^{
        ...
        // DXMT requires wine-8.0 or greater
        if ([fm fileExistsAtPath:[NSString stringWithFormat:@"%@/lib/wine/x86_32on64-unix/ntdll.so",self.wswineBundlePath]]) {
            [dxmtCheckBoxButton setEnabled:NO];
            [dxmtCheckBoxButton setState:NO];
        }

        // Disable all renders for legacy engines
...
- (IBAction)iconToUseBrowseButtonPressed:(id)sender
{
    ...
    NSString* wrapperIconPath = [NSString stringWithFormat:@"%@/Contents/Resources/Configure.icns",wrapperRealPath];
...
- (IBAction)d3dmetalButtonPressed:(id)sender
{
    [dxmtCheckBoxButton setState:NO];
    [dxvkCheckBoxButton setState:NO];
    [portManager setPlistObject:@([dxmtCheckBoxButton state]) forKey:WINESKIN_WRAPPER_PLIST_KEY_DXMT];
    [portManager setPlistObject:@([dxvkCheckBoxButton state]) forKey:WINESKIN_WRAPPER_PLIST_KEY_DXVK];
    [portManager setPlistObject:@([gptkCheckBoxButton state]) forKey:WINESKIN_WRAPPER_PLIST_KEY_D3DMETAL];
...
- (IBAction)dxvkButtonPressed:(id)sender
{
    [dxmtCheckBoxButton setState:NO];
    [gptkCheckBoxButton setState:NO];
    [portManager setPlistObject:@([dxmtCheckBoxButton state]) forKey:WINESKIN_WRAPPER_PLIST_KEY_DXMT];
...
- (IBAction)dxmtButtonPressed:(id)sender
{
    [gptkCheckBoxButton setState:NO];
    [dxvkCheckBoxButton setState:NO];
    [portManager setPlistObject:@([dxmtCheckBoxButton state]) forKey:WINESKIN_WRAPPER_PLIST_KEY_DXMT];
...
- (IBAction)winetricksRefreshButtonPressed:(id)sender
{
    ...
    [winetricksShowDownloadedColumn setState:([defaults boolForKey:@"DownloadedColumnShown"] ? NSControlStateValueOn : NSControlStateValueOff)];
    [winetricksShowInstalledColumn setState:([defaults boolForKey:@"InstalledColumnShown"] ? NSControlStateValueOn : NSControlStateValueOff)];
...
- (IBAction)winetricksUpdateButtonPressed:(id)sender
{
	...
	NSData *newVersion = [NSData dataWithContentsOfURL:[NSURL URLWithString:@"https://raw.githubusercontent.com/Sikarugir-App/winetricks/refs/heads/sikarugir/src/winetricks"] timeoutInterval:30];
	//if new version looks messed up, prompt the download failed, and exit.
	if (newVersion.length < 50) {
        [NSAlert showAlertOfType:NSAlertTypeError withMessage:@"Connection to the website failed. The site is either down currently, or there is a problem with your internet connection."];
...
- (IBAction)winetricksSelectAllButtonPressed:(id)sender
{
    ...
    [winetricksOutlineView setNeedsDisplay:YES];
}
...
- (IBAction)winetricksToggleColumn:(id)sender
{
	...
	if (sender == winetricksShowInstalledColumn) {
        [sender setState:(newState ? NSControlStateValueOn : NSControlStateValueOff)];
...
{
    NSString* portIconPath = [NSString stringWithFormat:@"%@/Contents/Resources/Configure.icns",self.wrapperPath];
...
- (IBAction)cEXESaveButtonPressed:(id)sender
{
	...
	//give done message
    [NSAlert showAlertOfType:NSAlertTypeSuccess withMessage:@"The Custom Exe Launcher has been made and can be found just inside the wrapper along with Configure.app.\n\nIf you want to be able to access it from outside of the app, just make and use an alias to it."];
...
- (IBAction)changeEngineUsedOkButtonPressed:(id)sender
{
	...
	system([[NSString stringWithFormat:@"/usr/bin/tar -C \"%@\" -xf \"%@/%@.tar.xz\"",WINESKIN_LIBRARY_ENGINES_FOLDER,WINESKIN_LIBRARY_ENGINES_FOLDER,newEngineName] UTF8String]);
	//delete old engine
    ...
    // Workaround explorer.exe hang with CX24.x
    system([[NSString stringWithFormat:@"/usr/bin/xattr -drs com.apple.quarantine \"%@\"",self.wrapperPath] UTF8String]);
...
- (IBAction)updateWrapperButtonPressed:(id)sender
{
	...
	if (files.count != 1) {
        [NSAlert showAlertOfType:NSAlertTypeError withMessage:@"There is an error with the installation of your Master Wrapper. Please update your Wrapper in Creator (a manual install of a wrapper for Creator will work too)."];
		return;
	}
    ...
    if ([NSAlert showBooleanAlertOfType:NSAlertTypeWarning withMessage:@"Are you sure you want to do this update? It will change out the wrappers main files with newer copies from whatever Master Wrapper you have installed with Creator. The following files/folders will be replaced in the wrapper:\nContents/Configure.app" withDefault:NO] == false) {
        return;
    }
    ...
    //edit Info.plist to new wrapper version, dump Template- and .app
    [portManager setPlistObject:[[masterWrapperName stringByReplacingOccurrencesOfString:@".app" withString:@""] stringByReplacingOccurrencesOfString:@"Template-" withString:@""] forKey:WINESKIN_WRAPPER_PLIST_KEY_WINESKIN_VERSION];
    ...
    //force delete Configure.app and copy in new
    [self replaceFile:@"/Contents/Configure.app" withVersionFromMasterWrapper:masterWrapperName];
