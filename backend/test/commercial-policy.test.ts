...
import { canActivate, consumeChallenge, isPromotable, newChallenge, offlineUntil, verifyDigest } from '../src/commercial-policy.js';
...
describe('commercial license and artifact policy', () => {
  ...
  it('expires and consumes challenges exactly once', () => {
    ...
    expect(() => consumeChallenge({ ...challenge, used: true }, 1_050)).toThrow();
  ...
  it('requires a matching artifact checksum before promotion', () => {
    expect(verifyDigest(new TextEncoder().encode('portside'), 'b3a4c4f3f4c2b5f8b75f4e8d53bd8b2e7a0d8d4c20c7d3e8f1a4f78e4cf8b6c0')).toBe(false);
