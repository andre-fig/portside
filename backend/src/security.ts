...
export function signLicenseToken(payload: Record<string, string | number>, privateKey: string): string {
  const header = Buffer.from(JSON.stringify({ alg: 'EdDSA', typ: 'PORTSIDE-LICENSE' })).toString('base64url');
