...
export class AppConfig {
  ...
  readonly artifactHosts = new Set(
    (process.env.PORTSIDE_ARTIFACT_HOSTS ?? "")
      .split(",")
      .map((v) => v.trim())
      .filter(Boolean),
  );
  readonly upstreamSigningKeys: Record<string, string> = (() => {
    try {
      return JSON.parse(
        process.env.UPSTREAM_SIGNING_KEYS_JSON ?? "{}",
  ...
  licensePrivateKey(): string {
    return required("LICENSE_SIGNING_PRIVATE_KEY_PEM");
  }
  licensePublicKey(): string {
    return required("LICENSE_SIGNING_PUBLIC_KEY_PEM");
  }
  licenseKeyId(): string {
    return required("LICENSE_SIGNING_KEY_ID");
  }
  manifestPublicKey(): string {
    return required("MANIFEST_SIGNING_PUBLIC_KEY");
  }
